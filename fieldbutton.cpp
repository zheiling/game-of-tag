...
FieldButton::FieldButton(int a_x, int a_y, int a_v, GameField *a_gptr) : x(a_x), y(a_y), v(a_v), gf(a_gptr), data(a_gptr->data) {
...
void FieldButton::setValue(FieldButton* fb, int v, bool matrix_sync) {
    ...
    fb->v = v;
    if (matrix_sync) {
        fb->data->setValue(fb->x, fb->y, fb->v);
    }
}
...
void FieldButton::pressCallback(Fl_Widget *b, void *user) {
    ...
    struct step_val sv;
    fb->data->getEmptyPosition(sv.e.x, sv.e.y);
    sv.v.x = fb->x;
    sv.v.y = fb->y;
